...
```{r}
encuesta_hogares <- read_sav("../Data/raw/Honduras/Data de la Encuesta de Hogares 2024_PDVF/Data de la Encuesta de Hogares 2024_PDVF.sav")
```
```{r}
encuesta_migracion <- read_sav("../Data/raw/Honduras/Encuesta_Migracion_Remesas_2023/Data de Emigrantes.sav")%>%
  clean_names()
```
...
hogares_summary <- encuesta_hogares %>%
  ...
  summarise(
    ...
    oih12_lps = sum(oih12_lps, na.rm = TRUE),
  ...
  summarise(
    ...
    # -----------------------------
    # Urban / Rural households (expandidos)
    # -----------------------------
    urban_households = sum(
      ifelse(urban_rural == "urban", peso_hogar, 0),
      na.rm = TRUE
    ),

    rural_households = sum(
      ifelse(urban_rural == "rural", peso_hogar, 0),
      na.rm = TRUE
    ),

    pct_urban_households =
      100 * urban_households / total_households,

    pct_rural_households =
      100 * rural_households / total_households,
...
write_csv(base_muni,
          "../Data/clean/municipality_summary_hnd.csv")
...
write_csv(emigration_summary,
          "../Data/clean/emigration_summary_hnd.csv")
```
